...
To support this we have 2 interface level views, one for the parameters eg zdemo_i_param and another for the output eg zdemo_i_output.
...
There are two behaviour files for both the interface and consumption parameter views. Theyre are examples zdemo_i_param and zdemo_c_param.

After generating the behaviour implementaation class, add the code for the execute action and setUser derivation eg zparam_implementation.  
...
The source code for a sample class file that uses the parameters can be found at zdemo_class.  
There is a helper class zparam_helper which has methods for clearing output and writing new lines.  
